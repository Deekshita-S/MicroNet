...
def _make_layer(block, in_channels, planes, nblocks, stride=1, constr_activation=None):
    layers = list()
    '''The downsample operation aims to match the dimensions of the input to the dimensions of the output when the stride of the convolutional layers is greater than 1. Used to implement a downsampling operation when the input dimensions change between the input and output of the block. This downsampling is typically required when the stride of the convolutional layers is greater than 1, resulting in spatial dimensions reduction.'''
...
class Resnet20(nn.Module):
    def __init__(self, block, quan_first=False, quan_last=False, constr_activation=None):
        ...
        self.quan_first = quan_first # to indicate if i/p shld be quantized before passing into resNet
        self.quan_last = quan_last # to indicate if o/p shld be quantized 
        self.quan_activation = constr_activation is not None
        ''' if you set quan_first to True and you also want to use a specific activation function for quantization, you would set constr_activation to the desired activation function, and quan_activation to True to enable quantization. If you want to use non-quantized activations, you would still set constr_activation to the desired activation function, but quan_activation would be set to False.'''
        if quan_first:
        ...
        if quan_last:
            ...
            self.fc = Linear(in_features=64, out_features=100, bias=True) # fully connected (FC) layer
            '''self.fc represents a fully connected layer that takes a flattened input with 64 features, applies a linear transformation to produce an output with 100 features, and optionally includes a bias term. This fully connected layer is typically used as the final layer in the neural network to map the extracted features to the desired output dimension, which in this case is 100.'''
        else:
...
This parameter indicates whether quantization should be applied to the input data (i.e., the first layer) of the network.
When quan_first is True, quantization is applied to the input data. This means that the input data is quantized before being processed by the network.
Setting quan_first to True allows quantization to be performed immediately on the input data, potentially reducing the computational cost and memory requirements throughout the network.'''
    return model
