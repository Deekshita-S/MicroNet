...
def get_optimizer(s_net, t_net, optimizer, lr_base, weight_decay, lr_scheduler, total_epoch, quan_activation=False,
    ...
    weight_bias = list()
    scale_weight = list()
    scale_activation = list() #Scale factors for activations are used in quantization techniques to scale the activations before quantization and then rescale them back after quantization. These scale factors are typically learned during the training process.
    ...
    # Learning rate schedulers are techniques used in training deep neural networks to adjust the learning rate during the training process.
